...
var deathCauseMap = map[string]DeathCause{
...
func (d DeathCause) String() string {
	...
	if d < MOD_UNKNOWN || d > MOD_GRAPPLE {
		return "UNKNOWN"
	}
	return names[d]
}

func ParseDeathCause(name string) (DeathCause, error) {
	cause, exists := deathCauseMap[name]
	if !exists {
		return MOD_UNKNOWN, errors.New("invalid DeathCause name")
	}
	return cause, nil
}
